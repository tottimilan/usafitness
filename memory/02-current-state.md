...
**Investigación TERMINADA** (11 agentes, 0 caídos) → `docs/plantillas/referencias-diseno/catalogo-patrones-2026.md` (37 patrones, 7 familias, 14 invariantes; hipótesis de 5 combinaciones: Cartel · Diagonal · Panel · Lámina · Franja — la última avisada como la más cercana a la rechazada Energía) y `docs/product/servicios-y-automatizacion-2026-08-27.md` (35 servicios en 5 capas, 15 nuevos, 22 descartes confirmados, 12 pasos de construcción, 6 preguntas). **Reparto del espacio HECHO** → `docs/plantillas/cinco/espacio-de-diseno.md`: Esquina (cuña, fachada recortada, cabecera que se compacta) · Estantería (chevrón cian/metal como baldas, tiras con snap) · Tablero (círculo punteado + pin, mosaico de datos sin hero) · Rótulo (script + mayúsculas, letra gigante sobre cian, sin efectos) · Recorrido (rayado 45° como progreso, una pregunta por pantalla). Matriz 5×5 de distinción, 15 invariantes. **El primer lanzamiento cayó por límite de sesión (5 diseñadores no arrancaron) y la síntesis por esquema demasiado grande → esquema adelgazado (devuelve arreglos, no rediseños) y relanzado con `resumeFromRunId`.** **CINCO PLANTILLAS DISEÑADAS Y CRITICADAS** (relanzado: 18 agentes, 0 caídos) → `docs/plantillas/cinco/plantillas.md` + `plantillas-resultado.json`. Veredictos: las 5 viven con arreglos; Estantería en libertad condicional (miniatura «apenas» vs Tablero y vs Recorrido → prueba de muerte triple antes de construir). Orden atado al norte: capa 0 (hecha) → **Rótulo** (demo pre-construida por defecto) → Esquina (2ª demo con foto de fachada) → Tablero → Estantería (condicional) → Recorrido (última: texto de marca). Arreglo transversal aceptado: **el minutero «cierra en X» se retira hasta que exista calendario de festivos (R3)**; los especímenes del artefacto pasan a «cierra a las 22:00» (hecho). 13 decisiones nuevas en memory/12. **Artefactos publicados:** «Cinco plantillas USA Fitness» → https://claude.ai/code/artifact/fed66a91-62c8-4a22-b2eb-06051f0c9ad5 (auditado: 0 fallos reales de contraste en claro y oscuro, 375 y 1340; 5 teléfonos de 375 px; foto real de Lagoh; fuentes cargadas) · «Guía del sistema USA Fitness» → https://claude.ai/code/artifact/977c7298-0a0d-499d-9910-00629cd60cd0 — junta Norte, Secciones F1, Inventario gen 2, Ronda 2 y Direcciones más marca, patrones, servicios y las cinco (456 elementos de texto, 0 fallos, ambos temas). **Siguiente:** confirmación del dueño sobre las cinco (y sus 13 decisiones), y después Loop A/B empezando por Rótulo.

## Sesión 6-sep — las ocho preguntas delegadas y el plan de Rótulo

**Encargo:** el dueño delegó las ocho preguntas que le quedaban («analízalas tú, busca en internet y en lo que tenemos, y respóndete tú») y pidió `/mm-plan` de Rótulo.

**Las ocho están respondidas** en `docs/product/decisiones-2026-09-06-ocho-preguntas.md`, siete por investigación con fuente primaria y dos resueltas por mí cuando el modelo se agotó. Lo que más cambia el sistema: **el calendario de festivos es por CENTRO COMERCIAL**, porque la Ley 1/2004 art. 5 da libertad de días y horas a los locales de menos de 300 m² que no son de un grupo grande — nuestras tiendas lo son, así que los calendarios autonómicos no les aplican. Con eso **el minutero «cierra en X» puede volver**, acotado a los días no marcados. Lo demás: Rótulo como demo por defecto de los 58 (0/58 prospectos publican foto: Esquina no puede ser la primera) · rótulos por regla determinista, 40/58 caben estrictos y 52/58 con corte ≤3 caracteres, medido con la tabla de avances real (k=0,927 em) · una sola prioridad · `placeId` verificado en build contra el CID (los 7 valores medidos y comprobados 7/7) · banco cerrado de textos de marca que la central firma una vez · vídeo propio, nunca YouTube (un embed cuesta ~1 MB y seis servidores antes del clic) · fuente del sistema con la pila corregida (empezaba por `-apple-system`, que en iPhone pinta San Francisco y no Helvetica Neue, y ponía Arial antes que Segoe UI, matando el Light en Windows).

**Rótulo repartido en rodajas** (`docs/features/rotulo/breakdown.md`): 1a datos · 1b fuentes · 1c ofertas · 1d hoy y festivos · 2 secciones de generación 2 · 3 la plantilla · 4 la demo. **Las dos primeras tienen plan escrito** (`.cursor/plans/2026-09-06-rotulo-1a-datos.md` y `-1b-fuentes.md`), son independientes y ninguna cambia un píxel de las ocho webs vivas. Medido y verificado durante la sesión: las dos fuentes de Rótulo generadas con fontTools desde un commit anclado pesan 5.124 y 4.520 bytes (22 KB con el CSS, frente al tope de 120), el descodificador de Place ID acierta en 7 de 7 y rechaza un CID pelado, y la cursiva del cartel real es fina —Allura o Sacramento—, no de pincel: comprobado comparando el render de tienda con las cinco candidatas.

**CONSTRUIDAS LAS RODAJAS 1a y 1b** (el dueño dijo «go»), rama `feat/rotulo-datos`. Suite armada: **220 pruebas, 220 pasan, 0 dormidas, 0 fallos** (la línea de partida eran 196 con 3 dormidas). Ninguna de las ocho webs vivas cambia, y hay un test que lo fija.

- **Datos:** `placeId` en 7/8 con guarda **aritmética** en el build (descodifica el Place ID y compara el CID que lleva dentro con el del enlace: un Place ID del centro comercial no compila) · `rotulo` en 8/8 con regla de dos líneas y mayúsculas · `prioridad` con `zonaMovil` que **intercambia** un bloque, nunca inserta · `fotoInterior` que tiene que ser de la propia galería · `pedir_resena` en el registro y en el clasificador · `scripts/place-id.mjs`, comprobado contra Google en vivo (7/7, salida 0).
- **Fuentes:** `archivo-expanded-black-rotulo.woff2` 5.120 B y `rotulo-script.woff2` (Allura) 4.520 B, generadas por `scripts/fuentes-rotulo.py` desde un commit anclado de google/fonts y **reproducibles byte a byte** tras fijar `SOURCE_DATE_EPOCH` (sin eso no salía: fontTools escribe la hora en la cabecera). 22 KB con el CSS frente al tope de 120. `usaFuenteBase` y `modo` entran en la interfaz `Template`, donde faltaban.
- **Tres mutaciones sobrevivieron y cambiaron el trabajo:** la guarda `origen === destino` era código muerto y se borró; el test del intercambio solo probaba un caso adyacente, donde intercambiar e insertar dan lo mismo, y se añadió el lejano; y una mutación del test de glifos mataba por el motivo equivocado (el esquema rechazaba antes) y hubo que repetirla.

**RODAJA 1d TAMBIÉN CONSTRUIDA** (rama `feat/rotulo-hoy-festivos`, PR #27). **235 pruebas, 235 pasan con la suite armada, 0 dormidas.** `franjaDeHoy` y `cierraEn` calculan con `Intl` y `Europe/Madrid` —el servidor va en tiempo universal y con `getDay()` la web diría «cerrado» con la tienda abierta, en las ocho a la vez y solo en producción—, y `estadoDeHoy` decide cuándo callar. **R3 queda cumplida y el minutero vuelve**, con la condición de que el centro tenga calendario vigente. `festivos.json` nace vacío y el build avisa de los ocho centros; `scripts/festivos.mjs` saca los doce o trece días del año que preguntar a cada uno.

**RODAJA 1c TAMBIÉN CONSTRUIDA** (rama `feat/rotulo-ofertas`, PR #28). **249 pruebas, 249 pasan con la suite armada, 0 dormidas.** Oferta central compartida más `ofertaPropia` por tienda, con precedencia entre ofertas VIVAS y **caducidad automática**: pasada la fecha deja de publicarse sin que nadie toque nada, porque una oferta vencida en la web de una tienda física no es un dato viejo sino una promesa que alguien intentará canjear en el mostrador. `procedencia` obligatoria (la oferta de la central se publica bajo el CIF del franquiciado — lección del «Hasta 20% dto.» retirado). Precios ocultos: el precio se BORRA en la función, no se oculta en la plantilla, y el esquema rechaza un precio metido dentro de la cifra grande.
