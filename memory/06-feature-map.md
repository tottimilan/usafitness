...
### FASE 3 — Cimientos técnicos ⏳ **EN CURSO** — 3.1, 3.2, 3.4, 3.5 y 3.7 cerradas · siguiente 3.6
...
| ⏭ 3.6 | **`Base.astro` + `Page.astro`**: el `<head>` está duplicado a mano entre `Landing.astro`, `[slug]/[doc].astro`, `index.astro` y ahora también `404.astro` — **cuatro copias**. Con páginas nuevas, la regla de `noindex` por host se aplicaría o no según qué fichero se copió: el fallo silencioso más caro posible en 7 dominios. **Sube de prioridad tras 3.5:** ahora que se pueden añadir URLs, cada página nueva es una quinta copia | M |
