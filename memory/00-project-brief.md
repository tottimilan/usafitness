...
- **Two distinct users — do not conflate:**
  - **(a) The store owner** = decides. Shown several proposals (templates + sections) and picks the style and sections for their store. Never touches a panel; the operator configures it. Each store is a **legally independent company with its own CIF**; some owners run 2–3 stores but as separate entities.
