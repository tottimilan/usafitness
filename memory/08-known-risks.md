...
|---|---|---|---|---|---|
| 0 | **Placeholder images block the 3 pending migrations.** Villanueva, Marineda and Las Rosas have zero real photos in the repo: `hero.jpg` and all 6 `tienda-N.jpg` are ~319-byte **SVG files with a `.jpg` extension** (solid `#3B6FC1`), generated by `scripts/generate-placeholders.js`, which hardcodes those exact three slugs. Verified in-browser: they do not decode (`ERROR_NO_DECODIFICA`), while Alcobendas/Vigo real files render fine. **NOT a production incident** — those three domains still serve the old WordPress site, so no visitor sees this. It is a blocker for migrating them | Medium | Confirmed (blocks migration, not live) | Pull the real photos from each store's WordPress `/wp-content/uploads/`; add a build check rejecting image files whose magic bytes do not match their extension | Open |
| 0b | `heroImage` feeds `og:image` and the JSON-LD `image`. **Alcobendas is live on Astro and its `hero.jpg` is a placeholder** (305 bytes, does not decode) → broken share preview on WhatsApp, a declared primary conversion channel. Vigo is fine | Medium | Confirmed — live | Replace `public/photos/alcobendas/hero.jpg` with a real image | Open |
