...
| 6 | **La red de tests no cubre render visual ni JS de cliente.** Un CSS que rompa la maqueta pasa los 60 tests. El aviso de cookies, la fachada del mapa y `ConversionTracking` se verifican como marcado, no como comportamiento | Medium | Medium | Verificación manual en navegador en cada cambio de UI (se está haciendo). Playwright queda descartado por ahora: pesa mucho para lo que falta | Open — aceptado |
