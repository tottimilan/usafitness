...
### 2026-08-27 — F0 cerrada: norte aprobado, catálogo extraído. Y tres decisiones del dueño

- **Norte y jerarquías: APROBADOS** («okey al norte y jerarquías») con cláusula de revisión explícita: se modifican más adelante si el trabajo lo pide, marcando versión y fecha. F0 cerrada el mismo día — el otro criterio (catálogo de usafitness.es) quedó cumplido con `docs/product/catalogo-usafitness-2026-08.json`: 137 categorías, 52 marcas, contadores verificados contra la investigación (1.683 productos), 8/8 marcas nuestras presentes.
- **`energia` en main: SE QUEDA, sin ofrecerse** (delegado por el dueño: «lo que tú consideres»). Razones: revertir removería main sin beneficio alguno (cero adoptantes = nadie la ve), su infraestructura (hoja CSS por plantilla, tipografía autoalojada, periferia) es la base de F3, y el coste de mantenerla muerta es ~0. Se retirará o rehará cuando F3 produzca la primera plantilla del método.
- **Los 8 actuales son PILOTOS del dueño, no clientes** — «los hice yo para verificar la viabilidad; solo les he realizado la web, no les ofrezco otros servicios». Cierra la suposición del 24-ago. Consecuencia mayor: hoy hay CERO ingresos recurrentes y los 8 son el escaparate del pitch, no una cartera. proceso-comercial §7 reescrito.
- **Precio del servicio: APLAZADO** («cuando tengamos todo, ya decidiré precio… desglose y análisis completo»). Los importes de memory/15 pasan a propuesta-ancla explícitamente marcada.
- **Files affected:** norte.md, proceso-comercial.md (§4, §7), memory/15 (cabecera), memory/12, memory/02 (tabla F), catalogo-usafitness-2026-08.json (nuevo).

### 2026-09-06 — Las ocho preguntas delegadas, respondidas; y Rótulo repartido en rodajas

- **Decisión:** el dueño delegó («analízalas tú y respóndete tú») las ocho preguntas abiertas de la síntesis de las cinco plantillas. Las ocho quedan respondidas con evidencia en `docs/product/decisiones-2026-09-06-ocho-preguntas.md`, y ninguna vuelve a su puerta salvo como confirmación de una lista o de una captura.
- **Las ocho, en una línea cada una:** Rótulo es la demo por defecto de los 58 y Esquina la segunda cuando llega una foto · los rótulos los escribe el operador con una regla determinista y el dueño confirma una tabla · una sola prioridad por tienda basta, y mueve un bloque en un hueco fijo · el calendario de festivos es POR CENTRO COMERCIAL, no por comunidad autónoma · un solo campo `placeId`, verificado en build contra el CID · el texto de marca sale de un banco cerrado que la central firma una vez · el vídeo es fichero propio, nunca YouTube · se acepta la fuente del sistema y se corrige la pila, que estaba mal escrita.
- **Razón de la que más cambia el sistema (festivos):** la Ley 1/2004 art. 5 da plena libertad de días y horas a los establecimientos de menos de 300 m² que no pertenecen a un grupo de distribución grande. Nuestras tiendas lo son y cada franquiciado es una sociedad independiente, así que los calendarios autonómicos de apertura en domingos y festivos **no les aplican**: lo que decide si el cliente llega a la puerta es el centro comercial. Consecuencia práctica: el minutero «cierra en X», retirado el 27-ago, **puede volver** acotado a los días no marcados.
- **Alternativas consideradas y por qué no:** Esquina como demo por defecto (0 de 58 prospectos publican foto y pedirla antes del primer contacto choca con el 57 % de no-respuesta medido) · dos grados de libertad de orden (60 combinaciones por diseñar, y en tres de cinco plantillas la segunda elección no haría nada) · guardar también el enlace corto `g.page/r/…` (medido: es el CID reempaquetado, dato duplicado con formato no documentado) · comprar la licencia web de Helvetica (por dominio y por páginas vistas, escalaría con cada tienda, y solo la vería la cuarta parte que ya la tiene gratis) · fachada de YouTube (su miniatura sale de un servidor de Google y hace preconexión a DoubleClick antes del clic: rompe R9).
- **Consecuencias:** cuatro campos nuevos de tienda, `zonaMovil` en el contrato de plantilla, dos fuentes propias generadas con fontTools, un fichero de festivos por centro, un banco de textos de marca y el evento `pedir_resena`. El epic queda repartido en `docs/features/rotulo/breakdown.md`; las dos primeras rodajas tienen plan escrito y son independientes entre sí.
- **Files affected:** docs/product/decisiones-2026-09-06-ocho-preguntas.md, docs/features/rotulo/breakdown.md, .cursor/plans/2026-09-06-rotulo-1a-datos.md, .cursor/plans/2026-09-06-rotulo-1b-fuentes.md, memory/02, memory/12, memory/16.

### 2026-09-06 — R3 se cumple: el minutero vuelve, y la unidad es el centro comercial

- **Decisión:** el estado en vivo («cierra en 2 h 15 min»), retirado el 27-ago por no tener calendario, vuelve con una condición construida en código: **solo se emite si el centro comercial de esa tienda tiene calendario y ese calendario cubre el día de hoy**. Sin él, o caducado, la página imprime la franja del día y nada más.
- **Razón de que la unidad sea el centro y no la comunidad:** la Ley 1/2004 de Horarios Comerciales, art. 5, da plena libertad de días y horas a los establecimientos de menos de 300 m² de superficie útil que no pertenezcan a un grupo de distribución sin consideración de pyme. Nuestras tiendas lo son y cada franquiciado es una sociedad independiente, así que los calendarios autonómicos de apertura en domingos y festivos **no les aplican**. Lo que decide si el cliente llega a la puerta es si abre el centro.
- **Consecuencia de diseño que simplifica:** no hace falta detectar si hoy es festivo. Si el centro no tiene calendario, ya hemos decidido no hablar; saber que hoy es fiesta no cambiaría nada. Se descartó por eso el estado «festivo sin dato» que estaba previsto.
- **Alternativas descartadas:** calendario por comunidad autónoma (no aplica por el art. 5) · calendario por tienda (el dato es del centro; dos tiendas del mismo centro lo comparten) · un fichero por centro (el módulo no puede tocar disco: lo cargan los tres cargadores, así que va un único JSON importado estáticamente).
- **Consecuencias:** `src/data/festivos.json` nace vacío y el build avisa de los ocho centros que faltan; `scripts/festivos.mjs` saca los doce o trece días del año que hay que preguntarle a cada uno. Rellenarlos es tarea del operador, unos quince minutos por centro y año.
- **Files affected:** src/data/horario.ts, src/data/festivos.ts, src/data/festivos.json, src/data/stores.ts, scripts/festivos.mjs, docs/metodologia/creacion-de-webs.md (R3), tests/datos.test.mjs.
