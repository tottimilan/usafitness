...
15. **[REPETIDO el 11-sep, y por eso está subrayado]** **Una mutación que mata por el motivo equivocado no demuestra nada** — 6-sep, ejecutando Rótulo. Para validar el test que impide que un rótulo use una letra fuera del subset de la fuente, puse un rótulo con un punto medio. El test se puso rojo… pero porque el ESQUEMA rechazaba ese rótulo antes de llegar al test. La mutación parecía buena y no probaba nada. Repetida quitando una letra que el esquema sí acepta, el test dijo lo que tenía que decir. **Regla:** al matar un test, leer el mensaje de error y comprobar que es EL que ese test emite; si el rojo viene de otra guarda, la mutación no cuenta.
16. **Una mutación que SOBREVIVE también es un hallazgo, y a veces el hallazgo es que sobra código** — misma sesión, tres de las mutaciones sobrevivieron. Una porque la guarda que mutaba era código muerto (intercambiar un elemento consigo mismo ya no hace nada): se borró. Otra porque el test solo probaba el caso ADYACENTE, donde intercambiar e insertar dan el mismo resultado: se añadió el caso lejano. La lección no es «escribir más tests», es que una mutación superviviente obliga a decidir cuál de las dos cosas está mal, el test o el código, y a veces es el código.
17. **El panel del navegador ESCALA la ventana emulada, y las medidas salen falsas** — 6-sep, Loop A de «Hazte socio»: pedí 375 px con `resize_window` y `innerWidth` devolvía 533; las capturas salían con la mitad de la pantalla en blanco porque el panel estaba oculto. Ninguna medida de una pieza visual vale así. **Regla:** toda medición y toda captura a un ancho concreto va dentro de un `<iframe>` del ancho exacto en una página propia, y lo primero que se comprueba es `innerWidth` del marco. Es la misma familia que el error 12 (el panel no es de fiar sin comprobarlo).
18. **Un `grep` que confirma por subcadena de otra palabra** — 11-sep, Loop A de «Empieza aquí». Tras recompilar, comprobé que el servidor ya servía el código nuevo con `grep -o 'estante'`: devolvió «estante» y di el reinicio por bueno. Lo que había encontrado era «estant**ería**», dentro de una frase de la FAQ que llevaba ahí desde el día anterior. El servidor seguía sirviendo la compilación vieja y estuve dos capturas mirando la versión anterior sin saberlo. **Regla:** una comprobación de «¿está el cambio?» se hace con el token completo y delimitado —`grep -c 'class="empieza-estante"'`, no `grep 'estante'`— y, si el resultado decide algo, se contrasta con un segundo hecho independiente (un contador que cambia, una cadena que debería HABER DESAPARECIDO). Es la misma familia que el error del test de FAQPage que se encontraba a sí mismo en un comentario.
19. **Matar un proceso por filtro en vez de por PID** — misma sesión: `taskkill //FI "WINDOWTITLE eq *"` no mató el servidor, el nuevo proceso no pudo tomar el puerto y falló en silencio contra un log que no estaba mirando. El síntoma fue el error 18. **Regla:** para reiniciar un servidor local, listar el PID (`Get-CimInstance Win32_Process`), matar por PID, y confirmar que el puerto responde **con contenido nuevo**, no solo con un 200.
20. **Recomendar una mecánica de git sin haberla visto funcionar en ESTE caso** — 11-sep. Le dije al dueño que mezclara con `--delete-branch` para aprovechar el reapuntado automático de GitHub en una pila de PRs. GitHub **cerró** la PR hija tres segundos después de mezclar la madre, en vez de reapuntarla; recuperarla exigió recrear la rama base (una PR cerrada no se reabre si su base ya no existe), reabrirla y reapuntarla a mano. **Regla:** una mecánica de una herramienta ajena solo se recomienda si se ha visto funcionar en la forma concreta en que se va a usar. En una pila, lo que no falla es reapuntar **todas** las hijas a `main` ANTES de mezclar nada; el borrado de ramas se hace al final y por separado.
21. **Una condición que solo vive en mi cabeza no es una condición** — misma sesión. Al ramificar el arreglo de seguridad decidí hacerlo desde `main` en vez de desde la punta de la pila, y anoté para mí que eso provocaría conflictos si se mezclaba antes que la pila. Se mezcló antes, y las cuatro PRs se rompieron a la vez. La decisión era defendible; el fallo fue no escribir la condición donde el dueño iba a leerla. **Regla:** si una elección mía tiene una consecuencia que depende de lo que haga otra persona, esa consecuencia va en el mensaje, no en el razonamiento.
22. **Alcanzabilidad medida solo contra el build local no vale** — misma sesión, reevaluando el triaje de seguridad. Dos de los cuatro hallazgos cambiaban de signo según dónde se midieran: `Host: USAFITNESSVIGO.COM` servía el host genérico en local y **no** en producción (Cloudflare pone la cabecera en minúscula), y el punto final del FQDN muere en el borde de Railway con su propio 404. Al revés también: `/_image` parecía cerrado mirando solo los ocho dominios y estaba abierto en cualquier otro host. **Regla:** cuando el veredicto de «¿nos alcanza?» depende de la infraestructura, se mide también CONTRA PRODUCCIÓN (`curl --resolve` con la cabecera cruda). Y un «no alcanzable porque el middleware lo reescribe» no es una defensa: es un efecto colateral, y se cierra explícitamente con un test.
23. **Las comillas invertidas dentro de un literal de plantilla, por tercera vez en dos días** — 11 y 12-sep. Se comieron `memory/16` en el cuerpo de una PR, cerraron un comentario de bloque con `**/mujer`, y cerraron DOS VECES el literal de `plantilla-rotulo-css.ts` al citar `align-self` y `clip-path` dentro de comentarios CSS. El patrón es siempre el mismo: cito un nombre de propiedad con comillas invertidas por costumbre de Markdown, dentro de un sitio donde esa comilla significa otra cosa. **Regla:** dentro de un literal de plantilla y dentro de una cadena de shell, los nombres de código se citan con comillas simples o sin nada. Y el error del build dice «Expected a semicolon» sin señalar la comilla, así que cuando salga eso en un fichero con un literal largo, lo primero que hay que contar son las comillas invertidas sin escapar.
24. **Un `&&` que hace de puerta y un `echo listo` que no significa nada** — 12-sep. Encadené `npm run build >/dev/null && matar-servidor; arrancar; echo listo`: el build falló, el `&&` se comió el reinicio, el servidor siguió sirviendo la compilación anterior y «listo» se imprimió igual. Estuve tres medidas creyendo que mi cambio de color no se aplicaba. **Regla:** si la salida de un comando se silencia, su código de salida tiene que gobernar TODA la cadena, no solo el eslabón siguiente; y un mensaje final de confirmación solo vale si comprueba el resultado, no si se limita a llegar al final del script. Misma familia que el error 18.
