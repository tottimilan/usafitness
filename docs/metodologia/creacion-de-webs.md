...
- **R3** Horario de HOY visible desde el primer scroll (dato estructurado). El estado en vivo («abierto · cierra 22:00») queda **CONDICIONADO**: solo con horario v2 + `holidays/<centro>.json` cargado con un año de antelación, y degradación segura (sin calendario de festivos → solo el horario del día, sin estado). Reconcilia el descarte razonado de memory/06 §5 — el día que el badge miente, alguien se desplaza a una persiana bajada.
  - **Construida el 2026-09-06** (`src/data/festivos.ts`, `src/data/horario.ts`). La condición se cumple así: el minutero solo se emite si el centro comercial de esa tienda tiene calendario y ese calendario cubre el día de hoy; sin él, o caducado, se imprime la franja del día y nada más. Así no hace falta saber si hoy es fiesta — si no lo sabemos, no lo decimos.
  - **La unidad es el CENTRO COMERCIAL, no la comunidad autónoma.** La Ley 1/2004 de Horarios Comerciales, art. 5, da plena libertad de días y horas a los establecimientos de menos de 300 m² de superficie útil que no pertenecen a un grupo de distribución grande. Nuestras tiendas lo son y cada franquiciado es una sociedad independiente, así que las órdenes autonómicas de apertura en domingos y festivos no les aplican: lo que decide si el cliente llega a la puerta es si abre el centro, y eso se pregunta. El calendario laboral solo sirve para saber por qué doce o trece días del año hay que preguntar (`node scripts/festivos.mjs <año>`).
  - Toda hora se calcula con `Intl` y `Europe/Madrid`, nunca con el reloj del servidor, que va en tiempo universal.
