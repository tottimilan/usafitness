...
Remediación: (a) monitor de uptime externo sobre los 7 dominios (0 código, plan gratuito) — cierra la detección de caídas; (b) endpoint `/health` que devuelva 200 y el nº de tiendas cargadas — **corregido el 2026-08-25: este documento decía `/_health`, y Astro excluye del enrutado todo `src/pages/_*`, así que esa ruta no existiría nunca. Ver `.cursor/plans/2026-08-25-medicion-y-observabilidad.md`**; (c) rellenar `ga4Id` + `googleSiteVerification` en las 4 tiendas vivas y verificar con `curl -s https://<dominio>/ | grep googletagmanager`. → `/mm-plan` para la slice; actualizar `memory/08` Technical #3 a *Mitigated* solo cuando el monitor emita avisos, no cuando esté dado de alta.
...
Hoy no hay riesgo aceptado; hay riesgo ignorado. Remediación: **`security-review`** para separar build-time (vite, esbuild, postcss, svgo, js-yaml) de runtime (`astro` sirve los 7 dominios en SSR), documentar en `docs/security/review-2026-08-25-deps.md`, y llevar los aceptados a `memory/08` como *Accepted* con justificación. Ojo: subir `astro` 6→7 es **salto mayor** y pasa por `/mm-plan` y `approval-gatekeeper`, no por `npm audit fix`. **⚠ Corregido el 2026-08-25 por el triaje (`85ec55e`): la afirmación de que las diez exigían `astro@7` + `@astrojs/node@11` era FALSA para el único aviso alcanzable.** `@astrojs/node@10.0.6` declara `peerDependencies: { astro: ^6.0.0 }` — patch del mismo major, ya aplicado. Cerraba el *cache poisoning* con `if-match` malformado, que era el único de los diez alcanzable desde internet: 500 → 412 en los estáticos, reproducido y fijado con test. Dato del triaje ya hecho aquí: GHSA-2pvr-wf23-7pc7 (SSRF, CVSS 7.5) **probablemente no aplica** — las 6 rutas del proyecto son `prerender = false`, no hay página de error prerenderizada. Queda al menos GHSA-8hv8-536x-4wqp (XSS reflejado vía slot name, CVSS 7.1) sin coartada.
